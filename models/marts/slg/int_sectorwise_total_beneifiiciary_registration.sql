with centres as (select * from {{ ref('int_centre_geographies') }}),
    ...
    calculating_total_onboarded as (
        ...
        from activities
        group by 1,2
        order by  sum(coalesce(cast(centre_onboarding as int),0) + coalesce(cast(home_onboarding as int),0) + coalesce(cast(follow_up as int),0) + coalesce(cast(community_engagement_onboarded as int),0)) desc
    )
select 
